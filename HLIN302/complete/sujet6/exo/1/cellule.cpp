...
bool Cellule::estDifferente(const Cellule& X)const{
  return !estEquivalente(X);  
}
bool Cellule::estEquivalente(const Cellule& X)const{
  return !estAvant(X) && !estApres(X);   
}
bool Cellule::estAvantOuEquivalente(const Cellule& X)const{
  return estAvant(X)||estEquivalente(X);
}
bool Cellule::estApresOuEquivalente(const Cellule& X)const{
  return estApres(X)||estEquivalente(X);
}
bool Cellule::estAvant(const Cellule& X)const{
  return((x<X.x)||(x==X.x && y<X.y));
}
bool Cellule::estApres(const Cellule& X)const{
  return X.estAvant(*this);
}
